...
The UI is created in Qt Designer. (Nowadays there is a newer alternative called Qt Creator, which may also work). The widgets for data entry are given names starting with the string ”data”. The app gathers input data from these widgets, and automatically derives variable names from the widget names by removing the prefix “data”. Currently, six types of data entry widgets are supported:
...
Widgets of type QLineEdit, QTextEdit and QCheckBox usually need no special configuration. The other widgets must be configured in Qt Designer.
...
* Configure the widget by opening the QSpinBox widget properties. Set ‘minimum’ and ‘maximum’ to reasonable minimum and maximum values, e.g. 200 mm and 600 mm. Set ‘suffix’ to ‘ mm’. Set the QAbstractSpinBox property ‘specialValueText’ to ‘Ei mitattu’. Set the default value (value property of QSpinBox) to the minimum (200).
...
If a variable is no longer needed, it can be simply deleted from the UI using Qt Designer. The corresponding column will still exist in the database table, but the program will not use it. Running “update_rom_schema.py” will report variables that are inconsistent between the UI and the database. It can add columns for new variables if necessary.
